from pm4py.algo.conformance.alignments.factory import apply as get_alignment
from pm4py.algo.conformance.alignments.factory import VERSION_STATE_EQUATION_A_STAR
...
from pm4py.objects.process_tree.util import parse as pt_parse
...
def empty_sequence_accepted(pt: ProcessTree) -> bool:
    alignment = __calculate_optimal_alignment(pt, Trace(), version=VERSION_STATE_EQUATION_A_STAR)
...
def __calculate_optimal_alignment(pt: ProcessTree, trace: Trace, version=VERSION_STATE_EQUATION_A_STAR):
    net, im, fm = convert_pt_to_petri_net(pt)
    return get_alignment(trace, net, im, fm, version=version)
